...
def test_excel_processing(content, tier):
    ...
    try:
        ...
        # Try to read Excel from bytes - skip header row
        df = pd.read_excel(io.BytesIO(content), usecols=[0], dtype={0: str}, header=0)
        ...
        # Process ZIP codes - get the actual column data, not header
        zip_column = df.iloc[:, 0]  # This gets all data rows, excluding header
        logger.info(f"Original ZIP column type: {zip_column.dtype}")
        logger.info(f"ZIP column sample: {zip_column.head(10).tolist()}")
        
        # Convert to string and clean
        zip_list = zip_column.astype(str).str.strip()
        logger.info(f"After string conversion: {zip_list.head(10).tolist()}")
        
        # Remove NaN values and invalid entries
        zip_list_clean = zip_list.dropna()
        # Remove any entries that are not valid ZIP codes (like 'nan', empty strings, etc.)
        zip_list_clean = zip_list_clean[zip_list_clean.str.match(r'^\d+
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=5000, debug=False))]
        logger.info(f"After cleaning: {zip_list_clean.head(10).tolist()}")
        
        # Zero fill to 5 digits
        zip_list_padded = zip_list_clean.str.zfill(5)
        logger.info(f"After zero padding: {zip_list_padded.head(10).tolist()}")
