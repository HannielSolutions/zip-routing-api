...
def test_excel_processing(content, tier):
    ...
    try:
        ...
        logger.info("=== Reading Excel file without header ===")
        df = pd.read_excel(io.BytesIO(content), header=None, dtype={0: str})
        logger.info(f"Shape: {df.shape}")
        logger.info(f"First 10 rows: {df.iloc[:10, 0].tolist()}")
        
        if df.empty:
            logger.warning(f"DataFrame is empty for {tier}")
            return set()
        
        # Skip first row (header) and get ZIP codes
        zip_column = df.iloc[1:, 0]  # Skip first row
        logger.info(f"ZIP codes (after skipping header): {zip_column.head(10).tolist()}")
        
        # Convert to string and clean
        zip_strings = zip_column.astype(str).str.strip()
        logger.info(f"As strings: {zip_strings.head(10).tolist()}")
        
        # Filter only numeric strings (valid ZIP codes)
        numeric_pattern = r'^\d+$'
        numeric_zips = zip_strings[zip_strings.str.match(numeric_pattern, na=False)]
        logger.info(f"Numeric only: {numeric_zips.head(10).tolist()}")
        
        # Pad to 5 digits
        padded_zips = numeric_zips.str.zfill(5)
        logger.info(f"Padded to 5 digits: {padded_zips.head(10).tolist()}")
        
        # Create final set
        zip_set = set(padded_zips)
        logger.info(f"Final set size: {len(zip_set)}")
        logger.info(f"Contains 07004: {'07004' in zip_set}")
        logger.info(f"Sample ZIPs: {list(zip_set)[:20]}")
